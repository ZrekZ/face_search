    ...
    public partial class Form1 : Form
    {
        ...
        private async void btnEgit_Click(object sender, EventArgs e)
        {
            await Task.Run(() =>
            {
                for (int i = 0; i < 10; i++)
                {
                    if (!recognition.SaveTrainingData(pictureBox2.Image, txtFaceName.Text)) MessageBox.Show("Ошибка", "При получении профиля произошла непредвиденная ошибка.", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Thread.Sleep(100);
